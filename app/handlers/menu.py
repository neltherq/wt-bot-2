...
async def start(message: Message):
    ...
    await message.answer(
        "✈️ Добро пожаловать в магазин WarThunder!\n"
        "Здесь ты найдёшь проверенные аккаунты любого ранга,\n"
        "быстрые покупки, мгновенную выдачу и приятные бонусы 💥\n\n"
        "🔥 Разделы:\n"
        "• 🎮 WarThunder — все доступные аккаунты\n"
        "• 👤 Профиль — баланс\n"
        "• 💳 Пополнить — моментальное пополнение через Lolz\n\n"
        "📢 Не забывай проверять новые лоты — обновления каждую неделю!\n\n"
        "💬 Если возникнут вопросы — свяжись с поддержкой через админа бота.",
...
async def support_text(message: Message):
    await message.answer(
        "📩 <b>Поддержка</b>\n"
        f"По вопросам обращайтесь: @bissmarq\n\n"
        "Просьба направлять обращения <b>одним сообщением</b>, "
        "четко и по существу описывая проблему. "
        "Это поможет быстрее рассмотреть запрос и ответить вам."
    )
